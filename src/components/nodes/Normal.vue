<template>
  <div class="m-2">
    <div class="input-group input-group-sm mb-1">
      <select class="form-select" v-model.number="properties.formatX">
        <option v-for="(format, i) in formats" :value="i">{{ format }}</option>
      </select>
      <select class="form-select" v-model.number="properties.formatY">
        <option v-for="(format, i) in formats" :value="i">{{ format }}</option>
      </select>
      <select class="form-select" v-model.number="properties.formatZ">
        <option v-for="(format, i) in formats" :value="i">{{ format }}</option>
      </select>
    </div>
    <div class="input-group input-group-sm mb-1">
      <label class="input-group-text" for="seamless">[-1 ,1] to [0, 1] remap</label>
      <div class="input-group-text">
        <input class="form-check-input mt-0" type="checkbox" id="seamless" v-model.boolean="properties.remap">
      </div>
    </div>
    <Slot :binding="bindings[0]" v-slot="{ connected }">
      <div class="input-group input-group-sm">
        <Slider v-model="properties.scale" :disabled="connected" label="Scale" />
      </div>
    </Slot>
    <Slot :binding="bindings[1]">
      <div class="input-group input-group-sm">
        <span class="input-group-text">Bump</span>
      </div>
    </Slot>
    <Slot :binding="bindings[2]">XYZ</Slot>
  </div>
</template>

<script lang="ts">
import shader from "./Normal.wgsl?raw" with { type: "text" };
...
const bindings = [
  ...
  { id: "bump",   index: 1, format: 1, output: false },
  { id: "output", index: 2, format: 3, output: true },
];
const formats = ["X", "Y", "Z", "-X", "-Y", "-Z", "0", "1"];
let pipeline;

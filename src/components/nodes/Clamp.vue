<template>
  <div class="m-2">
    ...
    <Slot :binding="bindings[1]" v-slot="{ connected }">
      <div class="input-group input-group-sm">
        <Slider v-model="properties.min" :disabled="connected" label="Min" />
      </div>
    </Slot>
    <Slot :binding="bindings[2]" v-slot="{ connected }">
      <div class="input-group input-group-sm">
        <Slider v-model="properties.max" :disabled="connected" label="Max" />
      </div>
    </Slot>
...
<script lang="ts">
import shader from "./Clamp.wgsl?raw" with { type: "text" };
...
const bindings = [
  ...
  { id: "min", index: 1, format: 0, output: false },
  { id: "max", index: 2, format: 0, output: false },

  ...
  <div class="m-2">
    <Slot :binding="bindings[0]" v-slot="{ connected }">
      <div class="input-group input-group-sm">
        <Slider v-model="properties.r" :disabled="connected" label="R" />
      </div>
    </Slot>
    <Slot :binding="bindings[1]" v-slot="{ connected }">
      <div class="input-group input-group-sm">
        <Slider v-model="properties.g" :disabled="connected" label="G" />
      </div>
    </Slot>
    ...
    <Slot :binding="bindings[4]" output>RGBA</Slot>
    <Slot :binding="bindings[5]" output>RGB</Slot>
    <Slot :binding="bindings[6]" output>RG</Slot>
  </div>
</template>

<script lang="ts">
import shader from "./Combine.wgsl?raw" with { type: "text" };
import { utilsShader, getNodePropertiesData } from "./../../utils.ts" with { type: "text" };
import { watch } from "vue";

const bindings = [
  { id: "r",    index: 0, format: 1, output: false },
  { id: "g",    index: 1, format: 1, output: false },
  { id: "b",    index: 2, format: 1, output: false },
  { id: "a",    index: 3, format: 1, output: false },
  { id: "rgba", index: 4, format: 4, output: true },
  { id: "rgb",  index: 5, format: 3, output: true },
  { id: "rg",   index: 6, format: 2, output: true },
];

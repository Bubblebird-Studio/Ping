<template>
  <div class="m-2">
    <Slot :binding="bindings[0]">UVW</Slot>
    <Slot :binding="bindings[1]">U</Slot>
    <Slot :binding="bindings[2]">V</Slot>
    <Slot :binding="bindings[3]">W</Slot>
  </div>
</template>

<script lang="ts">
import shader from "./Uvw.wgsl?raw" with { type: "text" };
import { utilsShader, getNodePropertiesData } from "./../../utils.ts" with { type: "text" };

const defaultProperties = {};
const bindings = [
  { id: "uvw", index: 0, format: 3, output: true },
  { id: "u",   index: 1, format: 1, output: true },
  { id: "v",   index: 2, format: 1, output: true },
  { id: "w",   index: 3, format: 1, output: true },
];
let pipeline;
